...
> **TL;DR**: Extract text → Translate sequentially (简体中文 only, NO parallel agents) → Review each chapter with parallel agent → Build EPUB with cover + summary + glossary. Run everything without asking. Output: `workspace/<书名>.epub`
...
Every translated word must be in **Simplified Chinese (简体中文)**. Traditional Chinese characters (繁體字) are forbidden. Before saving each chapter, scan for traditional characters and replace them with simplified equivalents. See `references/translation-style.md` for the full traditional→simplified lookup table.
...
**Exception**: The review agent (see Phase 2, Step 8) runs in the background on a completed chapter — that is allowed and does not violate sequential translation.
...
Input file (.txt/.epub/.pdf)
    ...
    → Review each chapter with parallel agent (quality checks + style fixes)
...
| `scripts/quality_check.py` | Deterministic quality checks on translated chapters | `python <skill_root>/scripts/quality_check.py` |
...
Create `workspace/reviews/` directory.
...
Also check `workspace/reviews/` — apply any unreviewed chapter alerts before continuing.
...
4. No translationese: avoid excessive 的, 被, 当...的时候, 和-everywhere, long modifier chains

### The loop — execute all chapters without pausing

**CRITICAL**: Steps 6-8 use the **Write tool** directly, NOT Python. There are only THREE Python commands in the entire translation process: (1) extraction script, (2) cover fetch, (3) EPUB build. Everything else — including all state file updates — uses Read/Write tools only. This means ZERO Bash permission prompts during the translation loop.
...
**Step 3**: Read `references/translation-style.md` to refresh translation guidelines (Read tool)

**Step 4**: Translate into Simplified Chinese. Follow the guidelines in `references/translation-style.md`. Pay special attention to:
- Break long modifier chains (the #1 priority)
- Convert 被-passive to active voice
- Drop unnecessary pronouns
- Replace 当...的时候 constructions
- Vary sentence length
- Reduce 的 density

Use glossary names. Add new characters/terms to glossary.

**Step 5**: Scan the translated text for traditional Chinese characters. Replace any found with simplified equivalents. Reference the table in `references/translation-style.md`.

**Step 6**: Save translation to `workspace/translated/chapter_NNNN.md` (Write tool)

**Step 7**: Update `workspace/summary.json` using the **Write tool** (NOT Python — write the JSON directly):
```json
{
  "last_chapter": N,
...
**Step 8**: Update `workspace/glossary.json` using the **Write tool** (NOT Python — write the JSON directly). Add any new characters, places, or terms discovered in this chapter:
```json
{
...
**Step 9**: Spawn a review agent for this chapter using the Agent tool. This runs in the background — do NOT wait for it:
```
Agent(
  subagent_type="general-purpose",
  run_in_background=true,
  description="Review chapter N translation quality",
  prompt="Review the translation of chapter N for quality. Follow the review process in references/review-prompt.md.

1. Read references/review-prompt.md for the full review rubric
2. Read workspace/extracted/chapters/chapter_NNNN.txt (English source)
3. Read workspace/translated/chapter_NNNN.md (Chinese translation)
4. Read workspace/glossary.json
5. Run: python scripts/quality_check.py workspace/translated/chapter_NNNN.md --glossary workspace/glossary.json
6. Score the chapter on all 4 dimensions (naturalness, register match, dialogue quality, translationese)
7. Edit workspace/translated/chapter_NNNN.md directly to fix clear issues:
   - Replace any traditional characters with simplified
...
8. If terminology was corrected, update workspace/glossary.json
9. Write review summary to workspace/reviews/chapter_NNNN_review.json
10. If you find a plot/structure inconsistency, write workspace/reviews/chapter_NNNN_ALERT.json"
)
```

**Step 10**: Report: "Chapter N/M done."
...
### Before proceeding to EPUB build

After all chapters are translated, check `workspace/reviews/` for any `*_ALERT.json` files. If present, read the alerts and address any structural/plot inconsistencies flagged by the review agents before building the EPUB.
...
- `references/review-prompt.md` — 4-dimension rubric (naturalness, register, dialogue, translationese)
- `scripts/quality_check.py` — deterministic checks (traditional chars, 的 density, 被-passive, etc.)

For tuning the translation prompt offline, see:
- `scripts/tune_prompt.py` — automated prompt tuning orchestrator
- `scripts/eval_metrics.py` — Layer 1: automated metric aggregation
- `scripts/eval_rubric.py` — Layer 2: model-as-judge scoring
- `scripts/eval_ab.py` — Layer 3: blind A/B comparison
- `references/eval-rubric.md` — 5-dimension evaluation rubric
- `test-corpus/` — representative text samples for tuning
...
# Run quality checks on a translated chapter
python scripts/quality_check.py workspace/translated/chapter_0001.md --glossary workspace/glossary.json

# Run Layer 1 evaluation (aggregated metrics)
python scripts/eval_metrics.py workspace/translated/ --glossary workspace/glossary.json

# Run Layer 3 evaluation (blind A/B comparison)
python scripts/eval_ab.py source.txt translation_v1.md translation_v2.md --runs 3
